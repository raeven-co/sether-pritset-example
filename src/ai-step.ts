...
//   tokenized payload in -> AI-assisted output (which may echo tokens) out.
// ============================================================================

import type { InvoicePayload, JsonValue } from './types.ts';

export interface TemplateAssistantResult {
  suggestedPlaceholders: string[];
  draftCoverNote: string;
  templateAdvice: string;
}

export async function runAiStep(
  tokenizedPayload: InvoicePayload,
): Promise<TemplateAssistantResult> {
...
function simulateTemplateAssistant(
  payload: InvoicePayload,
): TemplateAssistantResult {
  return {
    suggestedPlaceholders: collectPlaceholders(payload),
    draftCoverNote:
...
function collectPlaceholders(node: JsonValue, prefix = ''): string[] {
  if (Array.isArray(node)) {
    const first = node[0];
    return first === undefined ? [] : collectPlaceholders(first, `${prefix}[]`);
  }
